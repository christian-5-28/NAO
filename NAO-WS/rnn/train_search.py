...
import model_search
import data
from utils import batchify, save_checkpoint, generate_arch, parse_arch_to_seq, parse_seq_to_arch, build_arch, \
...
parser.add_argument('--child_max_seq_len_delta', type=int, default=20, help='max sequence length')
parser.add_argument('--num_intermediate_nodes', type=int, default=11)
parser.add_argument('--num_operations', type=int, default=4)
...
def train():
    ...
    if continue_train_child:
        ...
        child_model = model_search.RNNModelSearch(ntokens, child_params['emsize'], child_params['nhid'],
                                                  child_params['nhidlast'],
                                                  child_params['dropout'], child_params['dropouth'],
                                                  child_params['dropoutx'],
    ...
    if continue_train_child:
        ...
        if 't0' in optimizer_state['param_groups'][0]:
            child_optimizer = torch.optim.ASGD(child_model.parameters(), lr=child_params['lr'], t0=0, lambd=0.,
                                               weight_decay=child_params['wdecay'])
        else:
            child_optimizer = torch.optim.SGD(child_model.parameters(), lr=child_params['lr'],
                                              weight_decay=child_params['wdecay'])
    ...
    if continue_train_controller:
        ...
        controller_optimizer = torch.optim.Adam(controller_model.parameters(), lr=controller_params['lr'],
    ...
    while True:
        ...
        if child_params['arch_pool'] is None:
            arch_pool = generate_arch(controller_params['num_seed_arch'],
                                      num_nodes=args.num_intermediate_nodes,
                                      num_operations=args.num_operations)  # [[arch]]
        ...
        for e in range(child_params['eval_every_epochs']):
            child_epoch += 1
            model_search.train(train_data, child_model, parallel_child_model, child_optimizer, child_params,
                               child_epoch)
        ...
        # Evaluate seed archs
        valid_accuracy_list = model_search.evaluate(val_data, child_model, parallel_child_model, child_params,
                                                    eval_batch_size)
                ...
                with open(os.path.join(child_params['model_dir'], 'arch_pool'), 'w') as fa_latest:
                    with open(os.path.join(child_params['model_dir'], 'arch_pool.perf'), 'w') as fp_latest:
                        for arch, perf in zip(old_archs, old_archs_perf):
                            ...
                            fa.write('{}  {}\n'.format(arch, genotype))
                            fa_latest.write('{}  {}\n'.format(arch, genotype))
                            fp.write('{}\n'.format(perf))
                            fp_latest.write('{}\n'.format(perf))

        best_genotype = get_genotype(old_archs[0],
                                     num_nodes=args.num_intermediate_nodes,
                                     operations_list=PRIMITIVES)

        logging.info('BEST ARCH OF EPOCH {} IS: {}'.format(child_epoch, best_genotype))
        logging.info('WITH VALIDATION PPL: {}'.format(old_archs_perf[0]))

        if child_epoch >= child_params['train_epochs']:
            logging.info('Training finished!')
            logging.info('BEST ARCH END SEARCH IS: {}'.format(best_genotype))
            logging.info('WITH VALIDATION PPL: {}'.format(old_archs_perf[0]))
            return best_genotype

        # Train Encoder-Predictor-Decoder
        # [[arch]]
        encoder_input = list(map(lambda x: parse_arch_to_seq(x, num_nodes=args.num_intermediate_nodes), old_archs))
        ...
        controller_epoch = controller.train(encoder_input, encoder_target, decoder_target, controller_model,
                                            parallel_controller_model, controller_optimizer, controller_params,
                                            controller_epoch)
        ...
        new_archs = list(map(lambda x: parse_seq_to_arch(x,
                                                         num_nodes=args.num_intermediate_nodes), new_archs))  # [[arch]]
